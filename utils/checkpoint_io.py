...
def load_pretrained_weights(model, pretrained_weights, checkpoint_key, method):
    if os.path.isfile(pretrained_weights):
        ...
        if "scfs" in method :
            ...
            # remove `model.` prefix induced by torchextractor
...
def restart_from_checkpoint2(ckpt_path, is_tx_warp=False, **kwargs):
    """
    Re-start from checkpoint
    """
    if not os.path.isfile(ckpt_path):
        ...
        if key in checkpoint and value is not None:
            ...
            if is_tx_warp:
                for k in list(state_dict.keys()):
                    if "backbone" in k:
                        state_dict["module.backbone.model."+k[len("module.backbone."):]] = state_dict[k]
                        del state_dict[k]
            try:
