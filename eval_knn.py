...
def extract_feature_pipeline(args):
    ...
    if "vit" in args.arch:
        ...
        # if "fcl" in args.method and "dino" in args.method:
    ...
    model.cuda()
    if args.method == "mae":
        checkpoint_io.load_pretrained_weights_mae(model, args.pretrained_weights)
    else:
    ...
    train_features = extract_features(model, data_loader_train, args.arch, args.avgpool_patchtokens, args.use_cuda)
    print("Extracting features for val set...")
    test_features = extract_features(model, data_loader_val, args.arch, args.avgpool_patchtokens, args.use_cuda)
...
@torch.no_grad()
def extract_features(model, data_loader, arch="resnet50", avgpool_patchtokens=1, use_cuda=True, multiscale=False):
    ...
    for samples, index in metric_logger.log_every(data_loader, 10):
        ...
        if multiscale:
            ...
            if arch == "resnet50":
                ...
                output = model(samples, return_all_tokens=True) # cl_mode: class_token patch_token
                ...
                if avgpool_patchtokens == 0:
                    ...
                    # x[:, 1:].mean(1)
                    feats = torch.mean(output[:, 1:], dim=1)
                elif avgpool_patchtokens == 2:
                    # norm(x[:, 0]) + norm(x[:, 1:]).mean(1)
                    feats = output[:, 0] + torch.mean(output[:, 1:], dim=1)
                else:
                    assert False, "Unkown avgpool type {}".format(avgpool_patchtokens)
...
class ReturnIndexDataset(ImagenetLoader):
    def __getitem__(self, idx):
        img, lab = super(ReturnIndexDataset, self).__getitem__(idx)
        return img, idx

class ReturnIndexDataset_nori(ImagenetLoader):
    def __getitem__(self, idx):
        img, lab = super(ReturnIndexDataset_nori, self).__getitem__(idx)
        return img, idx
    ...
    if utils.get_rank() == 0:
        ...
        for k in args.nb_knn:
            top1, top5 = knn_classifier(train_features, train_labels,
                test_features, test_labels, k, args.temperature, args.num_labels, args.use_cuda)
